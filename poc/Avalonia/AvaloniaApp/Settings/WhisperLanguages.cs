...
{
    // Source: https://github.com/openai/whisper/blob/main/whisper/tokenizer.py LANGUAGES dict
    private static readonly (string Code, string Name)[] Raw = new[]
    {
    ...
    public static readonly (string Code, string Name)[] All = Raw
        .Select(t => (t.Code, Name: Capitalize(t.Name)))
        .OrderBy(t => t.Name)
        .ToArray();
    ...
    private static string Capitalize(string name)
    {
        ...
        // simple title case without Culture-specific complexity
        return string.Join(" ", name.Split(' ', StringSplitOptions.RemoveEmptyEntries)
            .Select(w => char.ToUpperInvariant(w[0]) + (w.Length > 1 ? w[1..] : string.Empty)));
    }
}